...
class Control(): #main class
 ...
 def __init__(self): #main function
     ...
     # self.pub = rospy.Publisher("/cmd_vel", Twist, queue_size=10) #publish message
        ...
        self.cmd_vel_pub = rospy.Publisher('/cmd_vel',Twist, queue_size=1) #publish message
        ...
        self.image_sub = rospy.Subscriber('/camera/color/image_raw',Image, self.image_callback) #subscribe message
        self.twist = Twist()
 ...
 def callback(self, msg): #function for obstacle avoidance
       ...
       #self.distance = 0.7
        ...
        #when no any obstacle near detected
            self.twist.linear.x = 0.5 # go (linear velocity)
          # self.twist.angular.z = 0.1 # rotate (angular velocity)
            self.twist.angular.z = 0.0 # rotate (angular velocity)
          ...
          # rospy.loginfo("Circling") #state situation constantly
            ...
            if msg.ranges[0] > self.distance and msg.ranges[15] > self.distance and msg.ranges[345] > self.distance and msg.ranges[45] > self.distance and msg.ranges[315] > self.distance:
                ...
                self.twist.linear.x = 0.5 #go
               #self.twist.angular.z = 0.1 #rotate

                self.twist.angular.z = 0.0 #rotate
...
if __name__ == '__main__':
    rospy.init_node('obstacle_avoidance_node') #initialize node
    Control() #run class
    rospy.spin() #loop it
